...
/*   Updated: 2023/10/19 15:56:06 by ele-sage         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	fill_objects(t_parse *parse, t_objects *object)
{
	...
	while (i < parse->nb_line)
	{
		...
		if (!components)
			return (false);
		if (ft_strncmp(parse->lines[i], "sp", 2) == 0)
			add_sphere(object, components);
		else if (ft_strncmp(parse->lines[i], "pl", 2) == 0)
			add_plane(object, components);
		else if (ft_strncmp(parse->lines[i], "cy", 2) == 0)
			add_cylinder(object, components);
		else if (ft_strncmp(parse->lines[i], "C", 1) == 0)
			object->camera = new_camera(components);
		else if (ft_strncmp(parse->lines[i], "L", 1) == 0)
			object->light = new_light(components);
		else if (ft_strncmp(parse->lines[i], "A", 1) == 0)
