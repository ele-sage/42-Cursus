...
bool bsp(Point const a, Point const b, Point const c, Point const point)
{
    ...
    Fixed const alpha = ((by - cy) * (px - cx) + (cx - bx) * (py - cy)) / ((by - cy) * (ax - cx) + (cx - bx) * (ay - cy));
    Fixed const beta = ((cy - ay) * (px - cx) + (ax - cx) * (py - cy)) / ((by - cy) * (ax - cx) + (cx - bx) * (ay - cy));
...
void displayPoint(Point const a, Point const b, Point const c, Point const point, int const width, int const height)
{
    ...
    if (inside)
        std::cout << "\033[1;32m P \033[0m(" << P[0]/10.0 << ", " << P[1]/10.0 << ") is inside the triangle" << std::endl;
    else
        std::cout << "\033[1;31m P \033[0m(" << P[0]/10.0 << ", " << P[1]/10.0 << ") is outside the triangle" << std::endl;
    std::cout << "\033[1;33m A \033[0m(" << A[0]/10.0 << ", " << A[1]/10.0 << ")" << " \033[1;33m B \033[0m(" << B[0]/10.0 << ", " << B[1]/10.0 << ")" << " \033[1;33m C \033[0m(" << C[0]/10.0 << ", " << C[1]/10.0 << ")" << std::endl << std::endl;
